...
export default function GetStartedModal({
  ...
  const handleGenerateCode = async () => {
    ...
    try {
      ...
      if (data.notWhitelisted || res.status === 403) {
        setStep("not_whitelisted");
        return;
      }
  ...
  const handleStartVerification = async () => {
    ...
    try {
      ...
      if (res.ok && data.verified && data.user) {
        ...
        // ONLY if existing user already has identity established in DB, skip to dashboard
        if (data.isExistingUser && data.hasIdentity && data.user.did) {
          setUser(data.user);
          if (onDone) {
            onDone();
          } else {
        ...
        // For all new users (or users without an identity):
        // ALWAYS advance to Agent Identity selection choice (Create / Import)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 select-none">
      ...
      <div className="relative w-full max-w-2xl bg-[#0c0d12] border border-white/[0.1] rounded-2xl overflow-hidden z-10 flex flex-col max-h-[92vh] overflow-y-auto">
        {/* Close Button (Hidden on Mobile UI and unwhitelisted/verifying steps) */}
        {step !== "verifying" && step !== "not_whitelisted" && (
          <button
            type="button"
            onClick={onClose}
        ...
        {/* --- STEP: NOT WHITELISTED VIEW (Red ASCII Cross) --- */}
        {step === "not_whitelisted" && (
          <div className="p-6 sm:p-8 flex flex-col items-center text-center max-w-lg mx-auto w-full">
            {/* Red ASCII Cross (No Glow) */}
            <div className="w-full bg-[#08090c] border border-white/[0.08] rounded-xl p-4 sm:p-6 mb-6 font-mono text-xs sm:text-sm leading-tight select-none text-center overflow-x-auto flex justify-center">
              <pre className="text-red-500 font-mono font-bold">
                {`   \\       /   
    \\     /    
     \\   /     
      \\ /      
   ...
   /       \\   `}
            ...
            <h3 className="text-xl sm:text-2xl font-semibold text-white tracking-[-0.02em] mb-6">
              You are not whitelisted to use agentK
            </h3>

            <button
              type="button"
              onClick={onClose}
              className="w-full bg-white/[0.06] hover:bg-white/[0.12] text-white border border-white/[0.1] font-semibold text-sm sm:text-[15px] py-3.5 px-6 rounded-xl flex items-center justify-center shadow-sm cursor-pointer transition-colors"
            >
              Dismiss
            </button>
          </div>
        ...
        {step === "connect" && (
          <div className="p-6 sm:p-8 flex flex-col items-center text-center max-w-lg mx-auto">
            ...
            <p className="text-xs sm:text-[13.5px] text-[#8e98a8] leading-relaxed mb-8">
              agentK silently monitors your public X contributions and pushes
              them to Flop Network under your personal DID.
            </p>
            <button
              type="button"
        ...
        {step === "input" && (
          <div className="p-6 sm:p-8 flex flex-col max-w-lg mx-auto w-full">
            ...
            <p className="text-xs sm:text-[13px] text-[#8e98a8] leading-relaxed mb-6">
              Provide your public username to generate your unique verification
              payload.
            </p>
            <div className="mb-6">
        ...
        {step === "code_generated" && (
          <div className="p-6 sm:p-8 flex flex-col max-w-lg mx-auto w-full">
            <h3 className="text-xl sm:text-2xl font-semibold text-white tracking-[-0.02em] mb-1.5 mt-1">
              {isExistingUser ? "Welcome back" : "Place Code in Bio"}
            </h3>
            <p className="text-xs sm:text-[13px] text-[#8e98a8] leading-relaxed mb-6 font-sans">
              {isExistingUser ? (
                <>
                  ...
                  is registered. Confirm your verification payload is in your
                  bio and click <strong className="text-white">Verify</strong>{" "}
                  to continue.
                </>
              ) : (
                <>
            ...
            <div className="mb-6">
              ...
              <p className="text-[11px] text-[#788290] mt-2.5 leading-relaxed font-sans">
                {isExistingUser ? (
                  <>
                    1. Confirm the payload above is in your X bio.
                    <br />
                    2. Click <strong className="text-white">Verify</strong> to
        ...
        {step === "create_identity" && (
          <div className="p-6 sm:p-8 flex flex-col max-w-lg mx-auto w-full">
            ...
            <p className="text-xs sm:text-[13px] text-[#8e98a8] leading-relaxed mb-6 font-sans">
              Enter a{" "}
              <strong className="text-white">13-character passphrase</strong> to
              encrypt your Ed25519 PKCS#8 PEM envelope with AES-256-CBC.
            </p>
            ...
            <div className="mb-6">
              ...
              <div className="relative flex items-center">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && createPassphrase.length >= 12) {
                ...
                <button
                  type="button"
                  onClick={() => setShowCreatePassphrase(!showCreatePassphrase)}
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <button
                ...
                disabled={createPassphrase.length < 12 || isCreatingIdentity}
                className="flex-[2] bg-[#17A2C6] hover:bg-[#1bb8df] disabled:opacity-40 disabled:cursor-not-allowed text-[#061d24] font-semibold text-sm py-3 px-4 rounded-xl shadow-md cursor-pointer transition-colors"
              >
                {isCreatingIdentity
                  ? "Generating Ed25519..."
        ...
        {step === "import_identity" && (
          <div className="p-6 sm:p-8 flex flex-col max-w-lg mx-auto w-full">
            ...
            <p className="text-xs sm:text-[13px] text-[#8e98a8] leading-relaxed mb-5 font-sans">
              Drag &amp; drop your{" "}
              <code className="text-white">identity.pem</code> file and enter
              your passphrase.
            </p>
        ...
        {step === "import_success" && (
          <div className="p-6 sm:p-8 flex flex-col max-w-lg mx-auto w-full">
            <div className="text-center mb-5">
              ...
              <p className="text-xs text-[#8e98a8]">
                Your existing Ed25519 identity has been decrypted and verified.
              </p>
            </div>
